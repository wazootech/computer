...
export default defineTool({
  description:
    "Verify Computer's GitHub App installation permission, approver-team access, and DeepSeek runtime without returning secrets. Reports which repository the session would work on and whether the App installation covers it; pass `repository` to check another owner/name.",
  inputSchema: z.object({
    checkDeepSeek: z.boolean().default(true),
    repository: z.string().min(1).optional(),
    teamSlug: z.string().min(1).default("team"),
  }),
  async execute({ checkDeepSeek, repository, teamSlug }, ctx) {
