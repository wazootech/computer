...
const SERVICE_DOCTOR = [
  ...
  "",
  "Services: 1",
  "  computer-discord: RUNNING, uptime 00:00:17, pid 4812",
  "",
  "Logs (last 10 lines):",
  "  {\"level\":\"info\",\"event\":\"bridge starting\"}",
  "  {\"level\":\"info\",\"event\":\"ready: computer-discord\",\"botId\":\"1551139973359476756\"}",
].join("\n");
...
describe("parseZoServiceList", () => {
  it("reads every service entry and ignores non-entry lines", () => {
    ...
    assert.equal(entries[1].label, "computer-discord");
    assert.equal(entries[1].workdir, "/home/workspace/users/etok/workspaces/wazootech/repos/computer");
    assert.equal(entries[1].entrypoint, "node --experimental-strip-types channels/discord/index.ts");
  });

  it("picks the single label match and refuses ambiguity", () => {
    const entries = parseZoServiceList(SERVICE_LIST);
    assert.equal(findZoService(entries, "computer-discord")?.serviceId, "svc_bridge123");
...
describe("parseZoServiceStatus", () => {
  it("reads the running state, uptime, and log tail", () => {
    const status = parseZoServiceStatus(SERVICE_DOCTOR, "computer-discord");
    assert.equal(status?.state, "RUNNING");
    assert.equal(status?.uptimeSeconds, 17);
    assert.match(status?.logs ?? "", /ready: computer-discord/u);
  });
